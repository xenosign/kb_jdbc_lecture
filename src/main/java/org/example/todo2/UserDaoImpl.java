...
public class UserDaoImpl implements UserDao {
    ...
    public UserVo loginUser(String user_id, String password) {
        // todo_user 테이블에서 매개 변수로 전달 받은 user_id 와 password 가 일치하는 데이터가 있는지 확인하며 되므로
        // SELECT 문의 검색 조건을 WHERE user_id=? AND password=? 요렇게 걸어서 쿼리를 작성한다
        ...
        // 지금은 로그인 정보 확인 전이므로 빈 값인 null 을 부여
        UserVo loginUser = null;

        // 로그인 할 때마다 다른 사용자 정보 값이 입력될 것이므로, PreparedStatement 를 사용하여 편리하게 쿼리를 변경하고 성능 최적화도 챙긴다        
        try(PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try(ResultSet rs = pstmt.executeQuery()) {
                ...
                // 적어도 보낸 쿼리를 만족하는 데이터가 1개 이상 존재한다는 말이므로 로그인에 성공한 케이스!
                while(rs.next()) {
