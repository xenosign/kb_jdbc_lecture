...
public class TodoProgram {
    public static void main(String[] args) {
        // todo-user DB의 작업을 처리하기 위해 UserDaoImpl 클래스를 인스턴스화
        ...
        // todo DB의 작업을 처리하기 위해 TodoDaoImpl 클래스를 인스턴스화
        TodoDaoImpl todoDao = new TodoDaoImpl();

        // 로그인한 사용자가 작성한 총 Todo 의 수를 보여주기 위해 todoDao.getTotalCount 를 수행
        // 로그인한 사용자 정보는 loggedInUser 에 저장되어 있으므로 loggedInUser.getUser_id() Getter 를 사용하여
        // 로그인한 사용자의 id 를 전달하여 확인
